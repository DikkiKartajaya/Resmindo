			...
			<table class="table table-bordered table-hover table-striped" id="tabel">
				...
				<tbody>
					...
					<tr>
						...
						<td><?= $status; ?></td>
						<td>
						<a href="index.php?ubah_makanan=<?= $data['id_masakan'] ?>" class="btn btn-warning btn-sm text-small" ><i class="fas fa-edit"></i></a>
						<a href="fungsi/hapusMakanan.php?id_masakan=<?= $data['id_masakan']; ?>"  class="btn btn-danger btn-sm text-small" onclick="return confirm('Yakin ingin menghapus data ini ?')"><i class="fas fa-trash"></i></a>
						</td>
					</tr>
				<?php endwhile; ?>
				</tbody>
