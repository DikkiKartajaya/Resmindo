<div class="container mt-3">
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <?php $i = 1;
            $order = mysqli_query($kon, "SELECT * FROM tb_order WHERE status_order = 0 and statusPesanan = 'uncooked'  ORDER BY id_order DESC ");
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                            ...
                            if ($level=="Koki"){ ?>
                            <a href="fungsi/orderSelesai.php?id=<?= $orders['id_order'] ?>" class="btn btn-warning btn-sm text-small" onclick="return confirm('Yakin Pesanan Telah Selesai ?')"><i class="fas fa-check"></i></a>
                            <?php } ?>
...
<div class="container mt-3">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <table class="table table-bordered table-hover table-striped" id="tabel2">
                <thead>
                    <tr> 
                        <th>No</th>
                        <th>No Pesanan</th>
            ...
            <?php $i = 1;
            $order = mysqli_query($kon, "SELECT * FROM tb_order WHERE status_order = 0 and statusPesanan = 'cooked' ORDER BY id_order DESC");
            foreach ($order as $orders) :
                $user_query =  mysqli_query($kon, "SELECT * FROM tb_user WHERE id_user = '$orders[id_user]'");
                $user = mysqli_fetch_assoc($user_query);
                $query_hartot = mysqli_query($kon, "SELECT sum(hartot_dorder) as hartot FROM tb_detail_order WHERE id_order = '$orders[id_order]'");
